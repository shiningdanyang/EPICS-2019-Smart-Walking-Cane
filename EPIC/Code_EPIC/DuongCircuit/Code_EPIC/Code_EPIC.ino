...
#define MAX_DISTANCE_LEFT 100 // Maximum distance we want to ping for (in centimeters). Maximum sensor distance is rated at 400-500cm.
// /////////////////////////////////////////////////////
...
void loop()
{
  read_mpu_6050_data();                                                //Read the raw acc and gyro data from the MPU-6050
  processing();
  if (abs(angle_pitch_buffer) > 200 || abs(angle_roll_buffer) > 200)    //detect fall
  {
    {
      Sim800l.print("AT+CMGF=1\r\n");
      delay(2000);
      Sim800l.print("AT+CMGS=\"0868611699\"\r\n");
      delay(2000);
      Sim800l.print("bao dong, toa do: https://www.google.com/maps/@16.07101,108.219941,19z ");
      Sim800l.print("\x1A");
      delay(4000);
      Sim800l.print("ATD0868611699;\r\n");
      delay(4000);
      while (1)
      {
        digitalWrite(BUZZER_PIN, HIGH);
        Serial.println("baodong");
      }
    }
  }


  // int distanceFake = newget(dist)
  ...
  if ((distanceFront != 0) || (distanceBottom != 0))
  {
    // phía trước có vật cản
    warningFront();
    return;
  }
  if ((distanceLeft != 0) && (distanceRight == 0))
  {
    // bên trái có vật cản
    warningLeft();
    notWarningRight();
    return;
  }
